"""
This DAG serves as an exmaple to show case how to create and to delete compute clusters in Azure Machine Learning.
"""
...
with DAG(
    ...
    compute1 = AmlCompute(
        name="af-test-cluster",
        size="Standard_D4s_v3",  # spec for a compute cluster
        min_instances=0,
        max_instances=1,
    )
    ...
    amlc_delete_1 = AzureMachineLearningDeleteComputeResourceOperator(
        task_id="delete_compute_cluster",
        conn_id=connection_id,
        compute_name=compute1.name,
    )

    compute2 = ComputeInstance(
        name="af-test-instance",
        size="Standard_D2s_v3",  # spec for a compute instance
    )
    amlc_create_2 = AzureMachineLearningCreateComputeResourceOperator(
        task_id="create_computer_instance",
        conn_id=connection_id,
        compute=compute2,
        waiting=True,
    )
    ...
    amlc_delete_2 = AzureMachineLearningDeleteComputeResourceOperator(
        task_id="delete_computer_instance",
        conn_id=connection_id,
        compute_name=compute2.name,
    )

    start_task = EmptyOperator(task_id="start")
    success_task = EmptyOperator(task_id="success")

    start_task >> amlc_create_1 >> iris_task >> amlc_delete_1 >> success_task
    start_task >> amlc_create_2 >> diabetes_task >> amlc_delete_2 >> success_task
